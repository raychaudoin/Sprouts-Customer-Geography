"""Disclosure-safe command line for the PIPE-03 protected binding."""
...
def _parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(
        description="Materialize the protected PIPE-03 Wisconsin development binding"
    )
    parser.add_argument("--repository-root", type=Path, default=Path.cwd())
    parser.add_argument(
        "--registry",
        type=Path,
        help="Exact protected-local authority registry; no discovery is performed",
    )
...
def _registry_argument(explicit: Path | None) -> Path | None:
    if explicit is not None:
        return explicit
    configured = os.environ.get("PIPE03_AUTHORITY_REGISTRY")
...
def main() -> int:
    ...
    try:
        resolver = load_authorized_registry(
            _registry_argument(arguments.registry),
        ...
        state = (
            "PIPE-03 exact-source binding blocked"
            if exc.code in dependency_blockers
            else "PIPE-03 implementation/conformance failed"
        )
        print(
            json.dumps(
                {
                    ...
                    "error_code": exc.code,
                    "isolated_sales_values_materialized": 0,
