"""Exact-handle protected resolver for MODEL-11."""
...
REGISTRY_ID = "MODEL11_PROTECTED_HANDLE_REGISTRY_V1"
...
REQUEST_FIELDS = frozenset(
    {
        ...
        "acs_b11001_source_handle",
        "tiger_source_handle",
        "data03_normalized_source_handle",
        "data03_verification_report_handle",
        "data03_ready_marker_handle",
        "model11_output_root_handle",
    }
)


class ProtectedHandleResolver:
    """Resolve only exact declared resources beneath authorized protected roots."""

    def __init__(self, registry: Mapping[str, Any], registry_path: Path, repository_root: Path):
        self.repository_root = repository_root.resolve()
        self.registry_path = registry_path.resolve()
        require(not _is_within(self.registry_path, self.repository_root), "PROTECTED_REGISTRY_INSIDE_REPOSITORY", "MODEL-11 registry must remain outside Git")
        require(registry.get("registry_id") == REGISTRY_ID and registry.get("version") == REGISTRY_VERSION, "PROTECTED_REGISTRY_IDENTITY_MISMATCH", "MODEL-11 registry identity/version mismatch")
        ...
        require(isinstance(request, Mapping) and set(request) == REQUEST_FIELDS, "DEVELOPMENT_REQUEST_INVALID", "MODEL-11 request fields differ from contract")
        ...
        semantic = {"registry_id": REGISTRY_ID, "version": REGISTRY_VERSION, "root_handles": sorted(self._roots), "resources": self._resources, "development_request": self.development_request}
    ...
    def load(cls, registry_path: Path, repository_root: Path) -> "ProtectedHandleResolver":
        ...
        require(path.is_file(), "AUTHORITATIVE_ACCESS_REGISTRY_UNRESOLVED", "explicit MODEL-11 registry is absent")
        try:
            document = json.loads(path.read_text(encoding="utf-8"))
        except (OSError, UnicodeError, json.JSONDecodeError) as exc:
            raise ConformanceError("PROTECTED_REGISTRY_INVALID", "MODEL-11 registry is unreadable") from exc
        require(isinstance(document, Mapping), "PROTECTED_REGISTRY_INVALID", "MODEL-11 registry must be an object")
    ...
    def resolve(self, handle: str, expected_kind: str, *, must_exist: bool = True) -> ResolvedHandle:
        ...
        if must_exist:
            require(candidate.is_dir() if expected_kind.endswith("_root") else candidate.is_file(), "PROTECTED_RESOURCE_UNRESOLVED", "exact protected resource does not resolve")
...
def load_authorized_registry(registry_path: Path | None, repository_root: Path) -> ProtectedHandleResolver:
    require(registry_path is not None, "AUTHORITATIVE_ACCESS_REGISTRY_UNRESOLVED", "no MODEL-11 protected registry was explicitly supplied")
